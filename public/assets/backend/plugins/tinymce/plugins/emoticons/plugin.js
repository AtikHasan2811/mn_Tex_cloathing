	...
	function getHtml() {
		...
		tinymce.each(emoticons, function(row) {
			...
			tinymce.each(row, function(icon) {
				var emoticonUrl = url + '/imgold/smiley-' + icon + '.gif';

				emoticonsHtml += '<td><a href="#" data-mce-url="' + emoticonUrl + '" data-mce-alt="' + icon + '" tabindex="-1" ' +
					'role="option" aria-label="' + icon + '"><imgold src="' +
			...
			onclick: function(e) {
				...
				if (linkElm) {
					editor.insertContent(
						'<imgold src="' + linkElm.getAttribute('data-mce-url') + '" alt="' + linkElm.getAttribute('data-mce-alt') + '" />'
					);
