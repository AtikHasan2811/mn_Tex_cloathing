...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function showDialog() {
		...
		win = editor.windowManager.open({
			...
			onSubmit: function() {
				...
				beforeObjects = editor.dom.select('imgold[data-mce-object]');
				editor.insertContent(dataToHtml(this.toJSON()));
				afterObjects = editor.dom.select('imgold[data-mce-object]');
	...
	function dataToHtml(data) {
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				...
				if (data.poster) {
					html += '<imgold src="' + data.poster + '" width="' + data.width + '" height="' + data.height + '" />';
				}
	...
	editor.addButton('media', {
		...
		onclick: showDialog,
		stateSelector: ['imgold[data-mce-object]', 'span[data-mce-object]']
	});
