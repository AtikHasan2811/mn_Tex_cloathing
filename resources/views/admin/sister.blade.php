...
{{--                                    <div class="row">--}}
                                        <div class="col text-right">
                                            <a style="width: 200px;" type="button" class="btn btn-outline-info  float-right " data-toggle="modal" data-target="#exampleModalinsert"  value="Add Photo">Add Sister</a>
...
{{--                                    <th>Photo</th>--}}
                                ...
                                <tbody>
                                @foreach($sister as $key=>$value)
                                    <tr>
                                        <th scope="row">{{$key+1}}</th>
                                        <td>{{ $value->name }}</td>
                                        <td>{{ $value->website }}</td>
                                        <td>{{ $value->contact_parsone }}</td>
                                        <td>{{ $value->phone }}</td>
                                        <td>{{ $value->email }}</td>
...
{{--                                        <td>--}}
{{--                                            <div class="zoom"><imgold src="{{url('uploads/about/'.$value->image)}}"--}}
...
{{--                                        </td>--}}
                                        <td class="text-center" style="width: 15%;">
                                            ...
                                            <a href="#"  class="btn btn-info waves-effect getdata" data-toggle="modal"
                                               ...
                                               data-contact_parsone="{{$value->contact_parsone}}"
                                            ...
                                            <button class="btn btn-danger waves-effect" type="button"
                                                    onclick="sisterdestroy({{$value->id}})">
                                                <i class="material-icons">delete</i>
                                            </button>
                                            <form id="delete-form-{{ $value->id }}"
                                                  action="{{ url('sisterdestroy/'.$value->id) }}" method="POST"
                                                  style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <form action="{{url('sisterupdate/')}}" enctype="multipart/form-data" method="post">
                @csrf
                <div class="modal-dialog modal-xl" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Update Sistern</h5>
                    ...
                    <input type="hidden" name="id"  id="id" class="form-control" value="" >
                            ...
                            <label for="email_address">Contact Parsone</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" name="contact_parsone"  id="contact_parsone" class="form-control" value="" >
                                </div>
                            </div>
    ...
    <!-- Modal  for insert-->
        <div class="modal fade modal-xl" id="exampleModalinsert" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <form  action="{{url('sister/store')}}" enctype="multipart/form-data" method="post">
                @csrf
            ...
            <script>
                $(document).ready(function(){
                    $(".getdata").click(function(){
                            ...
                            var contact_parsone = $(this).data('contact_parsone');
                        ...
                        if(status==1){
                            ...
                            $("#name").val(name);
                            $("#website").val(website);
                            $("#contact_parsone").val(contact_parsone);
                            $("#phone").val(phone);
                            $("#email").val(email);
